...
def test_valid_parser_arguments():
    ...
    arguments = parser.parse_args(['--file','./files/Dockerfile.sample','--config','./files/sample_config.json'])
    assert len(vars(arguments)) == 2

def test_no_config_passed(capsys):
    parser = docker_builder.createParser()
    with pytest.raises(SystemExit):
        parser.parse_args(['--file','./files/Dockerfile.sample'])
...
def test_no_file_passed(capsys):
    parser = docker_builder.createParser()
    with pytest.raises(SystemExit):
        parser.parse_args(['--config','./files/sample_config.json'])
    captured = capsys.readouterr()
